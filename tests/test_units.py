...
// 按品名再拆一層耗損原因（統計頁點開來看的那層）
const R2 = [
  {日期:'2026-08-01', 品類:'肉類', 品名:'雞胸肉', 原因:'報廢',     金額:180, 作廢:false},
  {日期:'2026-08-01', 品類:'肉類', 品名:'雞胸肉', 原因:'備料失誤', 金額:60,  作廢:false},
  {日期:'2026-08-02', 品類:'肉類', 品名:'雞胸肉', 原因:'報廢',     金額:120, 作廢:false},
  {日期:'2026-08-02', 品類:'蔬菜', 品名:'高麗菜', 原因:'過期',     金額:70,  作廢:false},
  {日期:'2026-08-02', 品類:'肉類', 品名:'雞胸肉', 原因:'試菜',     金額:500, 作廢:true},
];
const s2 = calc.summarize(R2, '2026-08-01', '2026-08-02');
const chick = s2.按品名[0];
eq('明細按原因分組、金額大的在前', chick.明細.map(x=>[x.名稱,x.金額,x.筆數]), [['報廢',300,2],['備料失誤',60,1]]);
eq('明細佔比是該品名內部的佔比', chick.明細.map(x=>x.佔比), [83.3,16.7]);
eq('明細排除作廢那筆', chick.明細.every(x=>x.名稱!=='試菜'), true);
eq('明細金額加總等於該品名金額', Math.round(chick.明細.reduce((a,x)=>a+x.金額,0)*100)/100, chick.金額);
eq('只有一種原因也給明細', s2.按品名.find(x=>x.名稱==='高麗菜').明細.map(x=>[x.名稱,x.佔比]), [['過期',100]]);
eq('按品類不帶明細', s2.按品類[0].明細, undefined);
eq('按原因不帶明細', s2.按原因[0].明細, undefined);
eq('原因沒填歸（未填）', calc.rank([{品名:'a', 原因:'', 金額:10}], '品名', '原因')[0].明細[0].名稱, '（未填）');
