...
const out = [];
const show = v => v === undefined ? '(undefined)' : v;   // JSON 表示不了 undefined，會把整個鍵丟掉
const eq = (name, got, want) => out.push({name, got: show(got), want: show(want), ok: JSON.stringify(got) === JSON.stringify(want)});
...
eq('其他的說明有存', calc.makeRecord({耗損量:1, 日期:'2026-08-01', 原因:'其他', 原因說明:'掉在地上'}, {品名:'x', 品類:'其他', 單位:'包', 單位成本:10}).原因說明, '掉在地上');
eq('改選別的原因殘留的說明不存', calc.makeRecord({耗損量:1, 日期:'2026-08-01', 原因:'報廢', 原因說明:'掉在地上'}, {品名:'x', 品類:'其他', 單位:'包', 單位成本:10}).原因說明, '');
...
eq('按品類明細是品名', s2.按品類[0].明細.map(x=>[x.名稱,x.金額,x.筆數]), [['雞胸肉',360,3]]);
eq('按品類明細佔比是該品類內部的', s2.按品類[0].明細[0].佔比, 100);
eq('按品類明細金額加總等於該品類金額', Math.round(s2.按品類[0].明細.reduce((a,x)=>a+x.金額,0)*100)/100, s2.按品類[0].金額);
eq('按原因明細是品名', s2.按原因.find(x=>x.名稱==='報廢').明細.map(x=>[x.名稱,x.金額]), [['雞胸肉',300]]);
eq('按原因明細也排除作廢', s2.按原因.some(x=>x.名稱==='試菜'), false);
eq('原因沒填歸（未填）', calc.rank([{品名:'a', 原因:'', 金額:10}], '品名', '原因')[0].明細[0].名稱, '（未填）');

// 「其他」原因的說明（展開層顯示）
eq('說明去重計數、多的在前', calc.notes([{原因說明:'冰箱跳電'},{原因說明:'掉在地上'},{原因說明:' 掉在地上 '},{原因說明:''},{}]),
   [{說明:'掉在地上', 筆數:2}, {說明:'冰箱跳電', 筆數:1}]);
eq('說明同樣多照先出現的順序', calc.notes([{原因說明:'b'},{原因說明:'a'}]).map(x=>x.說明), ['b','a']);
const R3 = [
  {日期:'2026-08-01', 品類:'肉類', 品名:'雞胸肉', 原因:'其他', 原因說明:'掉在地上',   金額:120, 作廢:false},
  {日期:'2026-08-01', 品類:'肉類', 品名:'雞胸肉', 原因:'其他', 原因說明:'掉在地上',   金額:120, 作廢:false},
  {日期:'2026-08-01', 品類:'肉類', 品名:'雞胸肉', 原因:'其他', 原因說明:'冰箱跳電',   金額:120, 作廢:false},
  {日期:'2026-08-01', 品類:'肉類', 品名:'雞胸肉', 原因:'其他', 原因說明:'作廢的說明', 金額:120, 作廢:true},
  {日期:'2026-08-01', 品類:'肉類', 品名:'雞胸肉', 原因:'報廢', 原因說明:'殘留的字',   金額:120, 作廢:false},
  {日期:'2026-08-01', 品類:'蔬菜', 品名:'高麗菜', 原因:'其他', 原因說明:'送錯貨',     金額:35,  作廢:false},
];
const s3 = calc.summarize(R3, '2026-08-01', '2026-08-01');
const c3 = s3.按品名.find(x=>x.名稱==='雞胸肉');
eq('其他附上該品名自己的說明', c3.明細.find(x=>x.名稱==='其他').說明, [{說明:'掉在地上', 筆數:2}, {說明:'冰箱跳電', 筆數:1}]);
eq('別的品名的說明不會混進來', c3.明細.find(x=>x.名稱==='其他').說明.some(x=>x.說明==='送錯貨'), false);
eq('其他以外的原因不帶說明', c3.明細.find(x=>x.名稱==='報廢').說明, undefined);
eq('另一個品名也拿到自己的說明', s3.按品名.find(x=>x.名稱==='高麗菜').明細[0].說明, [{說明:'送錯貨', 筆數:1}]);
const r3other = s3.按原因.find(x=>x.名稱==='其他');
eq('按原因的其他展開列的是品名', r3other.明細.map(x=>[x.名稱,x.金額]), [['雞胸肉',360],['高麗菜',35]]);
eq('其他底下每個品名掛自己的說明', r3other.明細[0].說明, [{說明:'掉在地上', 筆數:2}, {說明:'冰箱跳電', 筆數:1}]);
eq('其他底下另一個品名掛自己的說明', r3other.明細[1].說明, [{說明:'送錯貨', 筆數:1}]);
eq('按原因非其他的不掛說明', s3.按原因.find(x=>x.名稱==='報廢').明細.every(x=>x.說明===undefined), true);
eq('報廢那筆殘留的字不會被當說明', JSON.stringify(s3.按原因).indexOf('殘留的字'), -1);
eq('按品類不掛說明', s3.按品類.every(c=>c.明細.every(m=>m.說明===undefined)), true);
// （未填）的正規化：rank 與 summarize 兩邊寫法飄掉的話，紅的就是這兩條
eq('品名沒填在按原因底下也歸（未填）', calc.rank([{原因:'其他', 品名:'', 金額:10}], '原因', '品名')[0].明細[0].名稱, '（未填）');
eq('（未填）品名的其他說明也掛得上',
   calc.summarize([{日期:'2026-08-01', 品類:'其他', 品名:'', 原因:'其他', 原因說明:'沒選品名', 金額:10, 作廢:false}],
                  '2026-08-01','2026-08-01').按原因[0].明細[0].說明, [{說明:'沒選品名', 筆數:1}]);

// 子清單只列前 10 項，尾巴收成一列「其餘」
const L13 = Array.from({length:13}, (_,i)=>({名稱:'品'+i, 金額:130-i*10, 筆數:2, 佔比:0}));   // 總額 910，尾三項 60
eq('十項以內原樣不動', calc.topN(L13.slice(0,10), 10).length, 10);
eq('剛好十項不補其餘列', calc.topN(L13.slice(0,10), 10).some(x=>!!x.其餘), false);
eq('空清單不炸', calc.topN([], 10), []);
eq('超過十項只留十項加一列', calc.topN(L13, 10).length, 11);
eq('其餘列標剩下幾項', calc.topN(L13, 10)[10].其餘, 3);
eq('其餘列金額是被收起來那幾項的和', calc.topN(L13, 10)[10].金額, 60);
eq('其餘列筆數也要加總', calc.topN(L13, 10)[10].筆數, 6);
eq('其餘列佔比照 rank 的算法', calc.topN(L13, 10)[10].佔比, 6.6);
eq('收起來之後金額加總沒有漏', Math.round(calc.topN(L13, 10).reduce((a,x)=>a+x.金額,0)*100)/100, 910);
