...
// 顯示名稱帶縣市；STORES 的值是資料庫存的東西，改顯示名稱不要動到它
var STORE_LABEL = {
  '墨竹亭光復': '新竹光復店', '墨竹亭金山': '新竹金山店',
  '墨竹亭六張犁': '台北六張犁店', '墨竹亭美村': '台中美村店'
};
...
var calc = (function () {
  ...
  // 分組 key：空的一律歸「（未填）」。rank 與 summarize 共用，兩邊的寫法不會飄掉
  function bucket(v) { return v || '（未填）'; }

  // subField 給了就多算一層：每項掛一個「明細」陣列（同一函式遞迴，佔比是在該項內部的佔比）
  function rank(records, field, subField) {
    var map = {}, groups = {}, total = 0;
    for (var i = 0; i < records.length; i++) {
      var r = records[i], k = bucket(r[field]);
  ...
  // 子清單只列前 n 項，尾巴收成一列「其餘」（用「其餘」欄位當標記，畫面才分得出它不是真品項）。
  // 只做加總與四捨五入，不回頭重算金額。佔比照 rank 的算法（rest/total）——
  // 把尾巴各項已四捨五入的佔比相加會飄，項數多的時候差得出來。
  function topN(list, n) {
    if (!list || list.length <= n) return list || [];
    var total = 0, rest = 0, cnt = 0, i;
    for (i = 0; i < list.length; i++) total = Math.round((total + list[i].金額) * 100) / 100;
    for (i = n; i < list.length; i++) {
      rest = Math.round((rest + list[i].金額) * 100) / 100;
      cnt += list[i].筆數;
    }
    return list.slice(0, n).concat([{
      其餘: list.length - n,
      金額: rest,
      筆數: cnt,
      佔比: total ? Math.round(rest / total * 1000) / 10 : 0
    }]);
  }
  ...
  function summarize(records, from, to, store) {
    ...
    // 三張排行都再拆一層（統計頁點得開）：品類→品名、品名→原因、原因→品名
    var byCat = rank(rows, '品類', '品名');
    var byItem = rank(rows, '品名', '原因');
    var byReason = rank(rows, '原因', '品名');

    // 只有「其他」有說明欄（登記頁選其他才會出現、而且必填），別的原因就算殘留文字也不顯示。
    // 說明永遠住在「某品名 × 其他」這一格：按品名掛在子層的「其他」，
    // 按原因掛在「其他」底下的每個品名；按品類不掛——那層混了所有原因，
    // 掛上去會讓人以為整個品類都是這個原因。
    var others = rows.filter(function (r) { return r.原因 === '其他'; });
    function notesOf(item) {
      return notes(others.filter(function (r) { return bucket(r.品名) === item; }));
    }
    byItem.forEach(function (it) {
      it.明細.forEach(function (m) { if (m.名稱 === '其他') m.說明 = notesOf(it.名稱); });
    });
    byReason.forEach(function (rs) {
      if (rs.名稱 !== '其他') return;
      rs.明細.forEach(function (m) { m.說明 = notesOf(m.名稱); });
    });
  ...
  return {
    ...
    rank: rank, notes: notes, topN: topN, daily: daily, summarize: summarize, dateRange: dateRange, money: money,
...
if (typeof document !== 'undefined') document.addEventListener('DOMContentLoaded', function () {
  ...
  function applyStore(name) {
    ...
    rankOpen = {};   // 成本設定頁換門市不會觸發 s-store 的 change，這條路要自己清
  }
  ...
  function fillStoreSelect() {
    ...
    all.value = ''; all.textContent = '全部門市';
    el.appendChild(all);
    STORES.forEach(function (name) {
  ...
  $('s-store').addEventListener('change', function () { rankOpen = {}; renderStat(); });
  ...
  // 展開中的列。key＝「排行的 ul id｜名稱」——三張排行共用一份也不會撞名
  //（「肉類」當品類 vs 當品名是兩回事）。存在變數裡，重畫統計不會把展開的收回去；換店才清掉。
  var rankOpen = {};

  // 展開後的下一層清單：只有名稱／金額／佔比／筆數，不畫條狀圖（一列裡兩層條會看不出主次）。
  // 只列前 10 項，剩下的收成一列灰字「其餘 N 項」——一個原因底下可能有幾十個品名。
  function subRank(list) {
    var ul = document.createElement('ul');
    ul.className = 'subrank';
    calc.topN(list, 10).forEach(function (x) {
      var li = document.createElement('li');
      var rest = !!x.其餘;
      if (rest) li.className = 'r-rest';
      li.innerHTML =
        '<div class="r-top"><span class="r-name">' + (rest ? '其餘 ' + x.其餘 + ' 項' : x.名稱) + '</span><span class="r-amt">' + calc.money(x.金額) + '</span></div>' +
  ...
  function fillRank(el, list, limit, subLabel) {
    ...
    shown.forEach(function (r) {
      ...
      if (!canOpen) {
        ...
        var key = el.id + '｜' + r.名稱;
        li.className = 'can-open' + (rankOpen[key] ? ' is-open' : '');
        var head = document.createElement('button');
        head.type = 'button';
        head.className = 'r-head';
        head.setAttribute('aria-expanded', rankOpen[key] ? 'true' : 'false');
        head.innerHTML = body;
        head.addEventListener('click', function () {
          var nowOpen = !rankOpen[key];
          rankOpen[key] = nowOpen;
  ...
  function renderStat() {
    ...
    fillRank($('s-cat'), sum.按品類, 0, '品項');                       // 品類是固定選項，頂層不設上限
    fillRank($('s-item'), sum.按品名, itemRankOpen ? 0 : 10, '原因');   // 品名可能上百項，先給前 10
    fillRank($('s-reason'), sum.按原因, 0, '品項');
